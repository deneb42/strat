...
public class Communication {//extends Service {
	...
	public Communication(String host, int port) {
		try {
			...
			Log.v("Communication", "Unable to connect socket " + ex.getLocalizedMessage());
		}
	...
	public void stealStone(int who) {
		...
		try {
			...
			Log.e("Communication", "stealStone : IOException (" + e.getMessage() + ")");
		}
	}
				...
				while (running) {
					...
					case START_GAME:
						...
						for (int i=0; i < totalId; i++)
							alives[i] = true;
						Log.v("Communication", "Start game : " + iD + "/" + totalId);
						break;
					case STONE_QUANTITY:
						stones = istream.read();
					...
					case ACTION_GAUGE:
						...
						Log.v("Communication", "Action power update : " + actions);
						break;
					case OBTAIN_BONUS:
						bonus = istream.read();
					...
					case DISCONNECT:
						...
						if (tmp > 0 && tmp < alives.length)
							alives[tmp] = false;
						Log.v("Communication", "Player " + tmp + " is gone away");
						break;
					}
				}
